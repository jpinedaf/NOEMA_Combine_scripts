...
if os.path.isfile(file_source_catalogue) == False:
    file_source_catalogue = files("noema_combine").joinpath(file_source_catalogue)
    if os.path.isfile(file_source_catalogue) == False:
        raise FileNotFoundError(f"File not found: {config['catalogues']['source_catalogue']}")
...
if os.path.isfile(file_line_catalogue) == False:
    ...
    if os.path.isfile(file_line_catalogue) == False:
        raise FileNotFoundError(f"File not found: {config['catalogues']['line_catalogue']}")


list_source_name, list_source_key, list_source_out, list_RA, list_Dec, list_Vlsr, = \
    np.loadtxt(
...
ignorefiles = []
for key, item in config.items('file_handling'):#config['file_handling']['ignorefiles']
    if key.startswith('ignorefiles'):
        ignorefiles.append(item)


# load parameters used for the preparation of the data
line_name, QN, freq, name_str, QN_str, Lid, vel_width, vel_width_30m, vel_width_base_30m = np.loadtxt(
    file_line_catalogue,
    dtype="U",
    delimiter=",",
    quotechar='"',
    comments="#", usecols=(0, 1, 2, 3, 4, 9, 10, 13, 14),
    unpack=True)
...
def get_source_param(source_name: str) -> tuple[str, str, str, float, float, float]:
    ...
    if len(idx) == 0:
        raise ValueError(f'Source {source_name} not found in the catalogue: {source_print}')
    # return idx
    return list_source_name[idx], list_source_key[idx], list_source_out[idx], \
        list_RA[idx].astype(float), list_Dec[idx].astype(float), \
...
def line_reduce_30m(source_name:str, lines:str, QNs:str) -> None:
    source, source_find, source_out, ra0, dec0, vlsr  = get_source_param(source_name)
    ...
    for line_i, QN_i in zip(lines, QNs):
        ...
        # Define output
        outputfile = get_30m_file(source_out, line_name[index][0], QN[index][0])
